...
| Rich hover card | `CursorHover`/`CursorCard` (`lib/hover.tsx`) — a cursor-following portal card for rich detail (item/recipe/tech/module loadouts); `Icon` shows one by default. Not for plain text — that's `Tooltip`. |
